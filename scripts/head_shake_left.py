	...
	def macroHeadShake( self ):
		...
		if self.ALIVE:
			...
			while (not rospy.is_shutdown()) and (not self.mTT_INTERRUPT) and (_loop_count < self.repetition):
				...
				## VERSION 3: FROM UP --> DOWN --> 
				if headShake.v4:
					baseBehavior.monitorMoveToGP( self, str(self.shake_front_right), hp_gp=HP_RIGHT, delta_pp=3.5*DELTA_PP )

					baseBehavior.monitorMoveToGP( self, str(self.shake_right_left), ht_gp=HP_RIGHT, delta_pp=3.5*DELTA_PP )
